		...
		for (int i = 0; i < activeConnections.size(); i++)
		{
			...
			if (FD_ISSET(socket, &socketsReadyForReading))
			{
				...
				else if (result == 0)
				{
					...
					//BroadcastMessage("client dsiconnected",socket,activeConnections);
				...
				if (messageType == 1)
				{
					...
					//uint32_t senderNameLength = buffer->ReadUInt32LE();
					//std::string senderName = buffer->ReadString(senderNameLength);
					printf("PacketSize:%d\n MessageType:%d\n MessageLength:%d\n Message:%s\n", packetSize, messageType, messageLength, message.c_str());
		...
		if (count > 0)
		{
			if (FD_ISSET(listenSocket, &socketsReadyForReading))
			{
				...
				else
				{
					...
					//RecieveBroadCast(newConnection);
				}
			}
		}
	}
